...
static USBH_StatusTypeDef USBH_MIDI_InterfaceInit (USBH_HandleTypeDef *phost)
{	
  USBH_DbgLog("CUSBH_MIDI_InterfaceInit: Enter");

  USBH_StatusTypeDef status = USBH_FAIL;
	uint8_t interface = 0;
	MIDI_HandleTypeDef *MIDI_Handle;

	interface = USBH_FindInterface(phost, 255 /* JSB: Was USB_AUDIO_CLASS */, 0 /* JSB: Was USB_MIDISTREAMING_SubCLASS */, 0xFF);
	...
	else
	{
		...
		USBH_memset(phost->pActiveClass->pData, 0, sizeof(MIDI_HandleTypeDef)); // Added by JSB. Major bug fix! Caused data_tx_state to be invalid for some builds but not others!
...
{
  USBH_DbgLog("USBH_MIDI_InterfaceDeInit: Enter");
...
static USBH_StatusTypeDef USBH_MIDI_ClassRequest (USBH_HandleTypeDef *phost)
{   
  USBH_DbgLog("USBH_MIDI_ClassRequest: Enter");
...
static USBH_StatusTypeDef USBH_MIDI_Process (USBH_HandleTypeDef *phost)
{
  USBH_DbgLog("USBH_MIDI_Process: Enter");
...
{
  USBH_DbgLog("USBH_MIDI_Transmit: Enter");
...
{
  USBH_DbgLog("USBH_MIDI_Receive: Enter");
...
static void MIDI_ProcessTransmission(USBH_HandleTypeDef *phost)
{
  ...
  USBH_DbgLog("USBH_MIDI_Receive: data_tx_state=%d", MIDI_Handle->data_tx_state);
...
static void MIDI_ProcessReception(USBH_HandleTypeDef *phost)
{
  USBH_DbgLog("MIDI_ProcessReception: Enter");

  MIDI_HandleTypeDef *MIDI_Handle =  phost->pActiveClass->pData;
	USBH_URBStateTypeDef URB_Status = USBH_URB_IDLE;
	uint16_t length;

  USBH_DbgLog("USBH_MIDI_Receive: data_rx_state=%d", MIDI_Handle->data_tx_state);
